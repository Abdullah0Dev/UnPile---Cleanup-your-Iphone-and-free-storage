...
export default function HomePage() {
  ...
  const startTest = async () => {
    try {
      ...
      if (!granted) {
        Alert.alert('Permission required', 'Please grant photo access to test the analyzer.');
        return;
      }
      ...
      const subscription = ExpoPhotoAnalyzer.addListener('onProgress', (event: any) => {
        setProgress(event.progress);
        setCategory(event.category);
      });

      // 3. Start analysis (useAI = true)
      const result = await ExpoPhotoAnalyzer.analyzePhotos();
